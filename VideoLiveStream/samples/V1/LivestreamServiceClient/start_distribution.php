 ...
 */
function start_distribution_sample(string $formattedName): void
{
    // Create a client.
    $livestreamServiceClient = new LivestreamServiceClient();

    // Prepare the request message.
    $request = (new StartDistributionRequest())
        ->setName($formattedName);

    // Call the API and handle any network failures.
    try {
        /** @var OperationResponse $response */
        $response = $livestreamServiceClient->startDistribution($request);
...
function callSample(): void
{
    ...
    start_distribution_sample($formattedName);
}
