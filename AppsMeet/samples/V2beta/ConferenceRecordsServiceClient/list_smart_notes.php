...
// [START meet_v2beta_generated_ConferenceRecordsService_ListSmartNotes_sync]
 ...
 */
function list_smart_notes_sample(string $formattedParent): void
{
    // Create a client.
    $conferenceRecordsServiceClient = new ConferenceRecordsServiceClient();

    // Prepare the request message.
    $request = (new ListSmartNotesRequest())
        ->setParent($formattedParent);

    // Call the API and handle any network failures.
    try {
        /** @var PagedListResponse $response */
        $response = $conferenceRecordsServiceClient->listSmartNotes($request);

        /** @var SmartNote $element */
...
function callSample(): void
{
    ...
    list_smart_notes_sample($formattedParent);
}
// [END meet_v2beta_generated_ConferenceRecordsService_ListSmartNotes_sync]
