...
//   usage: php samples/V1/LanguageEntitySentimentText.php [--text_content "Grapes are good. Bananas are bad."]
...
function sampleAnalyzeEntitySentiment($textContent)
{
    ...
    try {
        $response = $languageServiceClient->analyzeEntitySentiment($document, ['encodingType' => $encodingType]);
        ...
        foreach ($response->getEntities() as $entity) {
            ...
            printf('Entity sentiment score: %s'.PHP_EOL, $sentiment->getScore());
            printf('Entity sentiment magnitude: %s'.PHP_EOL, $sentiment->getMagnitude());
