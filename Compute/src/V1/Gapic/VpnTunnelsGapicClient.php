 ...
 *     $pagedResponse = $vpnTunnelsClient->aggregatedList();
 ...
 * ```
 *
 * @deprecated Please use the new service client {@see \Google\Cloud\Compute\V1\Client\VpnTunnelsClient}.
 */
class VpnTunnelsGapicClient
{
     ...
     * $vpnTunnelsClient = new VpnTunnelsClient();
     * try {
     *     $operationResponse = $vpnTunnelsClient->delete();
    ...
    public function delete(array $optionalArgs = [])
    {
        ...
        if (isset($optionalArgs['vpnTunnel'])) {
            ...
            $requestParamHeaders['vpn_tunnel'] = $optionalArgs['vpnTunnel'];
        }
     ...
     * $vpnTunnelsClient = new VpnTunnelsClient();
     * try {
     *     $operationResponse = $vpnTunnelsClient->insert();
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
     *         // if creating/modifying, retrieve the target resource
     *     } else {
     ...
     * try {
     *     // Iterate over pages of elements
     *     $pagedResponse = $vpnTunnelsClient->list();
     *     foreach ($pagedResponse->iteratePages() as $page) {
     ...
     * $vpnTunnelsClient = new VpnTunnelsClient();
     * try {
     *     $operationResponse = $vpnTunnelsClient->setLabels();
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
     *         // if creating/modifying, retrieve the target resource
     *     } else {
     ...
     *     $operationName = $operationResponse->getName();
     *     // ... do other work
     *     $newOperationResponse = $vpnTunnelsClient->resumeOperation($operationName, 'setLabels');
     *     while (!$newOperationResponse->isDone()) {
     *         // ... do other work
     *         $newOperationResponse->reload();
     *     }
