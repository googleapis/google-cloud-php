 ...
 *     $pagedResponse = $targetInstancesClient->aggregatedList();
 ...
 * ```
 *
 * @deprecated Please use the new service client {@see \Google\Cloud\Compute\V1\Client\TargetInstancesClient}.
 */
class TargetInstancesGapicClient
{
     ...
     * $targetInstancesClient = new TargetInstancesClient();
     * try {
     *     $operationResponse = $targetInstancesClient->delete();
    ...
    public function delete(array $optionalArgs = [])
    {
        ...
        if (isset($optionalArgs['targetInstance'])) {
            $request->setTargetInstance($optionalArgs['targetInstance']);
            $requestParamHeaders['target_instance'] = $optionalArgs['targetInstance'];
        }
     ...
     * $targetInstancesClient = new TargetInstancesClient();
     * try {
     *     $operationResponse = $targetInstancesClient->insert();
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
     *         // if creating/modifying, retrieve the target resource
     *     } else {
     ...
     * try {
     *     // Iterate over pages of elements
     *     $pagedResponse = $targetInstancesClient->list();
     *     foreach ($pagedResponse->iteratePages() as $page) {
     ...
     * $targetInstancesClient = new TargetInstancesClient();
     * try {
     *     $operationResponse = $targetInstancesClient->setSecurityPolicy();
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
     *         // if creating/modifying, retrieve the target resource
     *     } else {
     ...
     *     $operationName = $operationResponse->getName();
     *     // ... do other work
     *     $newOperationResponse = $targetInstancesClient->resumeOperation($operationName, 'setSecurityPolicy');
     *     while (!$newOperationResponse->isDone()) {
     *         // ... do other work
     *         $newOperationResponse->reload();
     *     }
