 ...
 *     $pagedResponse = $packetMirroringsClient->aggregatedList();
 ...
 * ```
 *
 * @deprecated Please use the new service client {@see \Google\Cloud\Compute\V1\Client\PacketMirroringsClient}.
 */
class PacketMirroringsGapicClient
{
     ...
     * $packetMirroringsClient = new PacketMirroringsClient();
     * try {
     *     $operationResponse = $packetMirroringsClient->delete();
     ...
     *     // ... do other work
     *     $newOperationResponse = $packetMirroringsClient->resumeOperation($operationName, 'delete');
    ...
    public function delete(array $optionalArgs = [])
    {
        ...
        $requestParamHeaders = [];
        if (isset($optionalArgs['packetMirroring'])) {
            $request->setPacketMirroring($optionalArgs['packetMirroring']);
            $requestParamHeaders['packet_mirroring'] = $optionalArgs['packetMirroring'];
        }
     ...
     * $packetMirroringsClient = new PacketMirroringsClient();
     * try {
     *     $response = $packetMirroringsClient->get();
     * } finally {
     *     $packetMirroringsClient->close();
     * }
     * ```
     ...
     * $packetMirroringsClient = new PacketMirroringsClient();
     * try {
     *     $operationResponse = $packetMirroringsClient->insert();
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
     *         // if creating/modifying, retrieve the target resource
     *     } else {
     ...
     *     $operationName = $operationResponse->getName();
     *     // ... do other work
     *     $newOperationResponse = $packetMirroringsClient->resumeOperation($operationName, 'insert');
     *     while (!$newOperationResponse->isDone()) {
     *         // ... do other work
     *         $newOperationResponse->reload();
     *     }
     ...
     * try {
     *     // Iterate over pages of elements
     *     $pagedResponse = $packetMirroringsClient->list();
     *     foreach ($pagedResponse->iteratePages() as $page) {
     ...
     * $packetMirroringsClient = new PacketMirroringsClient();
     * try {
     *     $operationResponse = $packetMirroringsClient->patch();
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
     *         // if creating/modifying, retrieve the target resource
     *     } else {
     ...
     *     $operationName = $operationResponse->getName();
     *     // ... do other work
     *     $newOperationResponse = $packetMirroringsClient->resumeOperation($operationName, 'patch');
     *     while (!$newOperationResponse->isDone()) {
     *         // ... do other work
     *         $newOperationResponse->reload();
     *     }
