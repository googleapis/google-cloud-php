<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: google/cloud/compute/v1/compute.proto

namespace Google\Cloud\Compute\V1\CapacityAdviceRequestDistributionPolicy;

use UnexpectedValueException;

/**
 * Target distribution shape. You can specify the following values:ANY, ANY_SINGLE_ZONE, or BALANCED.
 *
 * Protobuf type <code>google.cloud.compute.v1.CapacityAdviceRequestDistributionPolicy.TargetShape</code>
 */
class TargetShape
{
    /**
     * A value indicating that the enum field is not set.
     *
     * Generated from protobuf enum <code>UNDEFINED_TARGET_SHAPE = 0;</code>
     */
    const UNDEFINED_TARGET_SHAPE = 0;
    /**
     * Picks zones for creating VM instances to fulfill the requested number
     * of VMs within present resource constraints.
     *
     * Generated from protobuf enum <code>ANY = 64972;</code>
     */
    const ANY = 64972;
    /**
     * Creates all VM instances within a single zone. The zone is selected
     * based on the present resource constraints.
     *
     * Generated from protobuf enum <code>ANY_SINGLE_ZONE = 61100880;</code>
     */
    const ANY_SINGLE_ZONE = 61100880;
    /**
     * Prioritizes acquisition of resources, scheduling VMs in zones where
     * resources are available while distributing VMs as evenly as possible
     * across selected zones to minimize the impact of zonal failure.
     *
     * Generated from protobuf enum <code>BALANCED = 468409608;</code>
     */
    const BALANCED = 468409608;
    /**
     * Default value, unused.
     *
     * Generated from protobuf enum <code>TARGET_SHAPE_UNSPECIFIED = 449316907;</code>
     */
    const TARGET_SHAPE_UNSPECIFIED = 449316907;

    private static $valueToName = [
        self::UNDEFINED_TARGET_SHAPE => 'UNDEFINED_TARGET_SHAPE',
        self::ANY => 'ANY',
        self::ANY_SINGLE_ZONE => 'ANY_SINGLE_ZONE',
        self::BALANCED => 'BALANCED',
        self::TARGET_SHAPE_UNSPECIFIED => 'TARGET_SHAPE_UNSPECIFIED',
    ];

    public static function name($value)
    {
        if (!isset(self::$valueToName[$value])) {
            throw new UnexpectedValueException(sprintf(
                    'Enum %s has no name defined for value %s', __CLASS__, $value));
        }
        return self::$valueToName[$value];
    }


    public static function value($name)
    {
        $const = __CLASS__ . '::' . strtoupper($name);
        if (!defined($const)) {
            throw new UnexpectedValueException(sprintf(
                    'Enum %s has no value defined for name %s', __CLASS__, $name));
        }
        return constant($const);
    }
}

