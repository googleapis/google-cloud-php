...
class BigtableClientTest extends GeneratedTest
{
    ...
    public function checkAndMutateRowTest()
    {
        ...
        // Mock request
        $rowKey = '122';
        $response = $gapicClient->checkAndMutateRow($rowKey);
    ...
    public function checkAndMutateRowExceptionTest()
    {
        ...
        $rowKey = '122';
        try {
            $gapicClient->checkAndMutateRow($rowKey);
    ...
    public function mutateRowTest()
    {
        ...
        $rowKey = '122';
        $mutations = [];
        $response = $gapicClient->mutateRow($rowKey, $mutations);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
    ...
    public function mutateRowExceptionTest()
    {
        ...
        $mutations = [];
        try {
            $gapicClient->mutateRow($rowKey, $mutations);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
    ...
    public function mutateRowsTest()
    {
        ...
        // Mock request
        $entries = [];
        $serverStream = $gapicClient->mutateRows($entries);
        $this->assertInstanceOf(ServerStream::class, $serverStream);
        $responses = iterator_to_array($serverStream->readAll());
        $expectedResponses = [];
        $expectedResponses[] = $expectedResponse;
    ...
    public function readModifyWriteRowTest()
    {
        ...
        $rowKey = '122';
        $rules = [];
        $response = $gapicClient->readModifyWriteRow($rowKey, $rules);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
    ...
    public function readModifyWriteRowExceptionTest()
    {
        ...
        $rules = [];
        try {
            $gapicClient->readModifyWriteRow($rowKey, $rules);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
    ...
    public function readRowsTest()
    {
        ...
        $transport->addResponse($expectedResponse3);
        // Mock request
        $serverStream = $gapicClient->readRows();
        $this->assertInstanceOf(ServerStream::class, $serverStream);
        $responses = iterator_to_array($serverStream->readAll());
        $expectedResponses = [];
        $expectedResponses[] = $expectedResponse;
    ...
    public function sampleRowKeysTest()
    {
        ...
        $transport->addResponse($expectedResponse3);
        // Mock request
        $serverStream = $gapicClient->sampleRowKeys();
        $this->assertInstanceOf(ServerStream::class, $serverStream);
        $responses = iterator_to_array($serverStream->readAll());
        $expectedResponses = [];
        $expectedResponses[] = $expectedResponse;
