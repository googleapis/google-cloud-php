 ...
 *
 * @param string $formattedDataStore The resource name of the Data Store, such as
 *                                   `projects/&#42;/locations/global/collections/default_collection/dataStores/default_data_store`.
 *                                   This field is used to identify the data store where to train the models. Please see
 *                                   {@see SearchTuningServiceClient::dataStoreName()} for help formatting this field.
 */
function train_custom_model_sample(string $formattedDataStore): void
{
    // Create a client.
    ...
    try {
        ...
        $response = $searchTuningServiceClient->trainCustomModel($request);
        $response->pollUntilComplete();

        if ($response->operationSucceeded()) {
            /** @var TrainCustomModelResponse $result */
...
function callSample(): void
{
    ...
    train_custom_model_sample($formattedDataStore);
}
