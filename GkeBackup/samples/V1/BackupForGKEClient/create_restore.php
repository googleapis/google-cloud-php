 ...
 * @param string $formattedBackup Immutable. A reference to the [Backup][google.cloud.gkebackup.v1.Backup] used as the source from which this Restore
...
function create_restore_sample(
    ...
    $restore = (new Restore())
        ->setBackup($restoreBackup);

    // Call the API and handle any network failures.
    try {
        /** @var OperationResponse $response */
        $response = $backupForGKEClient->createRestore(
            $formattedParent,
            $restore,
            $formattedBackup,
            $restoreId
...
function callSample(): void
{
    ...
    create_restore_sample($formattedParent, $formattedBackup, $restoreId);
}
