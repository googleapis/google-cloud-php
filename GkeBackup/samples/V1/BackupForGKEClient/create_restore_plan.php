 ...
 * @param string $formattedBackupPlan Immutable. A reference to the [BackupPlan][google.cloud.gkebackup.v1.BackupPlan] from which Backups may be used as the
 ...
 * @param string $formattedCluster    Immutable. The target cluster into which Restores created via this RestorePlan
...
function create_restore_plan_sample(
    ...
    $restorePlan = (new RestorePlan())
        ->setBackupPlan($restorePlanBackupPlan)
        ->setCluster($restorePlanCluster)
        ->setRestoreConfig($restorePlanRestoreConfig);

    // Call the API and handle any network failures.
    try {
        /** @var OperationResponse $response */
        $response = $backupForGKEClient->createRestorePlan(
            $formattedParent,
            $restorePlan,
            $formattedBackupPlan,
            $formattedCluster,
...
function callSample(): void
{
    ...
    $formattedBackupPlan = BackupForGKEClient::backupPlanName(
        '[PROJECT]',
        '[LOCATION]',
        '[BACKUP_PLAN]'
    );
    $formattedCluster = BackupForGKEClient::clusterName('[PROJECT]', '[LOCATION]', '[CLUSTER]');
