 ...
 *
 * @param string $formattedParent The parent space which owns the collection of pinned items
 *                                Format: `spaces/{space}`
 *                                Please see {@see ChatServiceClient::spaceName()} for help formatting this field.
 */
function list_message_pins_sample(string $formattedParent): void
{
    // Create a client.
    $chatServiceClient = new ChatServiceClient();

    // Prepare the request message.
    $request = (new ListMessagePinsRequest())
        ->setParent($formattedParent);

    // Call the API and handle any network failures.
    try {
        /** @var PagedListResponse $response */
        $response = $chatServiceClient->listMessagePins($request);

        /** @var MessagePin $element */
...
function callSample(): void
{
    ...
    list_message_pins_sample($formattedParent);
}
