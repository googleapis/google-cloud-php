...
class TemplatesServiceClientTest extends GeneratedTest
{
    ...
    public function createJobFromTemplateExceptionTest()
    {
        ...
        $request = new CreateJobFromTemplateRequest();
        try {
            $gapicClient->createJobFromTemplate($request);
    ...
    public function launchTemplateTest()
    {
        ...
        $response = $gapicClient->launchTemplate($request);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
    ...
    public function launchTemplateExceptionTest()
    {
        ...
        $request = new LaunchTemplateRequest();
        try {
            $gapicClient->launchTemplate($request);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
    ...
    /** @test */
    public function createJobFromTemplateAsyncTest()
    {
        $transport = $this->createTransport();
        $gapicClient = $this->createClient([
            'transport' => $transport,
