...
class JobsV1Beta3ClientTest extends GeneratedTest
{
    ...
    public function aggregatedListJobsTest()
    {
        ...
        $response = $gapicClient->aggregatedListJobs($request);
    ...
    public function aggregatedListJobsExceptionTest()
    {
        ...
        $request = new ListJobsRequest();
        try {
            $gapicClient->aggregatedListJobs($request);
    ...
    public function checkActiveJobsTest()
    {
        ...
        $response = $gapicClient->checkActiveJobs($request);
    ...
    public function checkActiveJobsExceptionTest()
    {
        ...
        $request = new CheckActiveJobsRequest();
        try {
            $gapicClient->checkActiveJobs($request);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
    ...
    public function snapshotJobTest()
    {
        ...
        $response = $gapicClient->snapshotJob($request);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
    ...
    public function snapshotJobExceptionTest()
    {
        ...
        $request = new SnapshotJobRequest();
        try {
            $gapicClient->snapshotJob($request);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
    ...
    /** @test */
    public function aggregatedListJobsAsyncTest()
    {
        $transport = $this->createTransport();
        $gapicClient = $this->createClient([
            'transport' => $transport,
        ...
        $transport->addResponse($expectedResponse);
        $request = new ListJobsRequest();
        $response = $gapicClient->aggregatedListJobsAsync($request)->wait();
        $this->assertEquals($expectedResponse, $response->getPage()->getResponseObject());
        $resources = iterator_to_array($response->iterateAllElements());
        $this->assertSame(1, count($resources));
        $this->assertEquals($expectedResponse->getJobs()[0], $resources[0]);
