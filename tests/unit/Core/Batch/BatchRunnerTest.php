...
use Google\Cloud\Core\Batch\JobSubmitInterface;
...
class BatchRunnerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->jobSubmitter = $this->prophesize(JobSubmitInterface::class);
    ...
    public function testSubmitItem()
    {
        ...
        $this->configStorage->unlock()
            ->willreturn(true)
            ->shouldBeCalledTimes(1);
        $this->jobSubmitter->submit('item', 1)
            ->shouldBeCalledTimes(1);
        $runner = new BatchRunner(
            $this->configStorage->reveal(),
            $this->jobSubmitter->reveal()
