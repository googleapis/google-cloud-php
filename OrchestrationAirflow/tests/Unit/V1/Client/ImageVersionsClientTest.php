...
class ImageVersionsClientTest extends GeneratedTest
{
    ...
    public function listImageVersionsTest()
    {
        ...
        $response = $gapicClient->listImageVersions($request);
    ...
    public function listImageVersionsExceptionTest()
    {
        ...
        $request = new ListImageVersionsRequest();
        try {
            $gapicClient->listImageVersions($request);
    ...
    /** @test */
    public function listImageVersionsAsyncTest()
    {
        $transport = $this->createTransport();
        $gapicClient = $this->createClient([
            'transport' => $transport,
        ...
        $transport->addResponse($expectedResponse);
        $request = new ListImageVersionsRequest();
        $response = $gapicClient->listImageVersionsAsync($request)->wait();
        $this->assertEquals($expectedResponse, $response->getPage()->getResponseObject());
        $resources = iterator_to_array($response->iterateAllElements());
        $this->assertSame(1, count($resources));
        $this->assertEquals($expectedResponse->getImageVersions()[0], $resources[0]);
