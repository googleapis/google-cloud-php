 ...
 *
 * @param string $formattedParent The ImageImport's parent. Please see
 *                                {@see VmMigrationClient::locationName()} for help formatting this field.
 * @param string $imageImportId   The image import identifier.
 ...
 *                                hyphen.
 */
function create_image_import_sample(string $formattedParent, string $imageImportId): void
{
    // Create a client.
    ...
    try {
        ...
        $response = $vmMigrationClient->createImageImport($request);
        $response->pollUntilComplete();

        if ($response->operationSucceeded()) {
            /** @var ImageImport $result */
...
function callSample(): void
{
    ...
    $imageImportId = '[IMAGE_IMPORT_ID]';

    create_image_import_sample($formattedParent, $imageImportId);
}
