 ...
 *                                    Specifies the repository to fetch the references from. Please see
 *                                    {@see SecureSourceManagerClient::repositoryName()} for help formatting this field.
 */
function fetch_refs_sample(string $formattedRepository): void
{
    // Create a client.
    $secureSourceManagerClient = new SecureSourceManagerClient();

    // Prepare the request message.
    $request = (new FetchRefsRequest())
        ->setRepository($formattedRepository);

    // Call the API and handle any network failures.
    try {
        /** @var PagedListResponse $response */
        $response = $secureSourceManagerClient->fetchRefs($request);
...
function callSample(): void
{
    ...
    fetch_refs_sample($formattedRepository);
}
