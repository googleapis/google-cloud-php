 ...
 *
 * @param string $formattedName The resource name of the automatic improvements.
 ...
 */
function get_automatic_improvements_sample(string $formattedName): void
{
    // Create a client.
    $automaticImprovementsServiceClient = new AutomaticImprovementsServiceClient();

    // Prepare the request message.
    $request = (new GetAutomaticImprovementsRequest())
        ->setName($formattedName);

    // Call the API and handle any network failures.
    try {
        /** @var AutomaticImprovements $response */
...
{
    $formattedName = AutomaticImprovementsServiceClient::automaticImprovementsName('[ACCOUNT]');

    get_automatic_improvements_sample($formattedName);
}
