 ...
 *
 * @param string $formattedName The name of the feed to enable.
 *                              Format:
 ...
 */
function enable_feed_sample(string $formattedName): void
{
    // Create a client.
    $feedsServiceClient = new FeedsServiceClient();

    // Prepare the request message.
    $request = (new EnableFeedRequest())
        ->setName($formattedName);

    // Call the API and handle any network failures.
    try {
        /** @var Feed $response */
        $response = $feedsServiceClient->enableFeed($request);
...
function callSample(): void
{
    ...
    enable_feed_sample($formattedName);
}
