 ...
 *
 * @param string $formattedParent The parent, which owns this collection of logs.
 *                                Format:
 ...
 */
function import_push_logs_sample(string $formattedParent): void
{
    // Create a client.
    ...
    $request = (new ImportPushLogsRequest())
    ...
    try {
        ...
        $response = $feedsServiceClient->importPushLogs($request);
...
{
    $formattedParent = FeedsServiceClient::feedName('[PROJECT]', '[LOCATION]', '[INSTANCE]', '[FEED]');

    import_push_logs_sample($formattedParent);
}
