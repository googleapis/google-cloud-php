...
class CloudChannelReportsServiceClientTest extends GeneratedTest
{
    ...
    public function fetchReportResultsTest()
    {
        ...
        $response = $gapicClient->fetchReportResults($formattedReportJob);
    ...
    public function fetchReportResultsExceptionTest()
    {
        ...
        $formattedReportJob = $gapicClient->reportJobName('[ACCOUNT]', '[REPORT_JOB]');
        try {
            $gapicClient->fetchReportResults($formattedReportJob);
    ...
    public function listReportsTest()
    {
        ...
        $response = $gapicClient->listReports($parent);
        $this->assertEquals($expectedResponse, $response->getPage()->getResponseObject());
        $resources = iterator_to_array($response->iterateAllElements());
        $this->assertSame(1, count($resources));
    ...
    public function listReportsExceptionTest()
    {
        ...
        $parent = 'parent-995424086';
        try {
            $gapicClient->listReports($parent);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
    ...
    public function runReportJobTest()
    {
        ...
        $incompleteOperation->setName('operations/runReportJobTest');
        $incompleteOperation->setDone(false);
        $transport->addResponse($incompleteOperation);
        $expectedResponse = new RunReportJobResponse();
        $anyResponse = new Any();
        $anyResponse->setValue($expectedResponse->serializeToString());
        $completeOperation = new Operation();
        $completeOperation->setName('operations/runReportJobTest');
        $completeOperation->setDone(true);
        $completeOperation->setResponse($anyResponse);
        $operationsTransport->addResponse($completeOperation);
        // Mock request
        $formattedName = $gapicClient->reportName('[ACCOUNT]', '[REPORT]');
        $response = $gapicClient->runReportJob($formattedName);
