...
// [START cloudlocationfinder_v1_generated_CloudLocationFinder_ListCloudLocations_sync]
 ...
 */
function list_cloud_locations_sample(string $formattedParent): void
{
    // Create a client.
    $cloudLocationFinderClient = new CloudLocationFinderClient();

    // Prepare the request message.
    $request = (new ListCloudLocationsRequest())
        ->setParent($formattedParent);

    // Call the API and handle any network failures.
    try {
        /** @var PagedListResponse $response */
        $response = $cloudLocationFinderClient->listCloudLocations($request);
...
function callSample(): void
{
    ...
    list_cloud_locations_sample($formattedParent);
}
// [END cloudlocationfinder_v1_generated_CloudLocationFinder_ListCloudLocations_sync]
