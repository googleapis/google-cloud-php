...
class UpdateReadmeSampleCommandTest extends TestCase
{
    ...
    {
        $this->root = sys_get_temp_dir() . '/google-cloud-php-tests-' . rand();
        $componentDir = $this->root . '/ClientSnippets';
        $filesystem = new Filesystem();
        $filesystem->mirror(__DIR__ . '/../../fixtures/component/ClientSnippets', $componentDir);
        ...
        file_put_contents($componentDir . '/.repo-metadata.json', json_encode([
        ...
        $application->add(new UpdateReadmeSampleCommand($this->root));
    ...
    public function testUpdateReadmeSample()
    {
        ...
        $this->assertStringContainsString(
            ...
            file_get_contents($this->root . '/ClientSnippets/README.md')
        );
    }

    public function testUpdateSampleWithNewVersion()
    {
        $v1SamplePath = $this->root . '/ClientSnippets/samples/v1/ClientSnippetsClient/an_rpc_method.php';
        $v2SampleDir = $this->root . '/ClientSnippets/samples/v2/ClientSnippetsClient';
        ...
        $v2SrcDir = $this->root . '/ClientSnippets/src/V2';
        if (!is_dir($v2SrcDir)) {
        ...
        $readme = file_get_contents($this->root . '/ClientSnippets/README.md');
        $this->assertStringContainsString(
