...
class Component
{
 ...
 \* Updates to the above are reflected here through a refresh process#';
        ...
        // Get the clients in the first version of the component
        $versions = $this->getApiVersions();
        $version = $versions ? array_shift($versions) : '';
        $gapicClients = [];
        $gapicClients += $this->getFilesInDir($this->path . '/src', $version, '*GapicClient.php');
        $gapicClients += $this->getFilesInDir($this->path . '/src', $version, '*BaseClient.php');
        ...
        foreach ($gapicClients as $gapicClient) {
            ...
            if (preg_match($regex, $gapicClientContent, $matches)) {
                $protos[] = dirname($matches[1]);
            }
        }

        return array_unique($protos);
    }
