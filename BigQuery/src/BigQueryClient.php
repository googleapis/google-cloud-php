...
class BigQueryClient
{
    ...
    /**
     * Create a duplicate of an already run job.
     *
     * Example:
     * ```
     * $table = $bigquery->dataset('my_dataset')->table('my_table');
     * $jobConfig = $table->extract('gs://my_bucket/my_object');
     *
     * $job = $table->runJob($job);
     *
     * // If the job failed, try again. In real usage, you should fix the errors before doing this.
     * if (isset($job['info']['status']['errorResult'])) {
     *     $duplicateJob = $bigquery->duplicateJob($job);
     *     $table->extract($duplicateJob);
     * }
     * ```
     *
     ...
     */
    public function duplicateJob($job, array $options = [])
    {
        if (is_string($job)) {
            $job = $this->job($job);
        } elseif (!($job instanceof Job)) {
            throw new \InvalidArgumentException("Argument must be a job ID or instance of Job.");
        }

        $info = $job->info($options);

        $typeMap = [
            'EXTRACT' => ExtractJobConfiguration::class,
            'LOAD' => LoadJobConfiguration::class,
            'COPY' => CopyJobConfiguration::class,
            'QUERY' => QueryJobConfiguration::class,
        ];

        $jobType = $info['configuration']['jobType'];
        switch ($jobType) {
            case 'EXTRACT':
            case 'COPY':
            case 'LOAD':
                $type = $typeMap[$jobType];
                $res = new $type(
            ...
            default:
                throw new \RuntimeException(sprintf(
                    "Unknown/unexpected job typs %s",
                    $info['configuration']['jobType']
                ));
                break;
        }
