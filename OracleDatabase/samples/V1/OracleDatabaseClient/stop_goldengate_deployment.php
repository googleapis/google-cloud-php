 ...
 */
function stop_goldengate_deployment_sample(string $formattedName): void
{
    // Create a client.
    $oracleDatabaseClient = new OracleDatabaseClient();

    // Prepare the request message.
    $request = (new StopGoldengateDeploymentRequest())
        ->setName($formattedName);

    // Call the API and handle any network failures.
    try {
        /** @var OperationResponse $response */
        $response = $oracleDatabaseClient->stopGoldengateDeployment($request);
...
function callSample(): void
{
    ...
    stop_goldengate_deployment_sample($formattedName);
}
