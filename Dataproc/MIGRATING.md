...
try {
    ...
    $response = $sessionControllerClient->getSession($request);
...
$response = $sessionControllerClient->getSession($request, $callOptions);
```
...
$request = GetSessionRequest::build($name);
$response = $sessionControllerClient->getSession($request);
```
