 ...
 * @group gapic
 */
class AppOptimizeClientTest extends GeneratedTest
{
    ...
    /** @return AppOptimizeClient */
    private function createClient(array $options = [])
    {
        $options += [
            'credentials' => $this->createCredentials(),
        ];
        return new AppOptimizeClient($options);
    }
    ...
    public function createReportTest()
    {
        ...
        $report = new Report();
        $reportDimensions = [];
        $report->setDimensions($reportDimensions);
        $reportMetrics = [];
        $report->setMetrics($reportMetrics);
    ...
    public function deleteReportTest()
    {
        ...
        // Mock request
        $formattedName = $gapicClient->reportName('[PROJECT]', '[LOCATION]', '[REPORT]');
        $request = (new DeleteReportRequest())->setName($formattedName);
        $gapicClient->deleteReport($request);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.cloud.appoptimize.v1beta.AppOptimize/DeleteReport', $actualFuncCall);
    ...
    public function getReportTest()
    {
        ...
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.cloud.appoptimize.v1beta.AppOptimize/GetReport', $actualFuncCall);
        $actualValue = $actualRequestObject->getName();
        $this->assertProtobufEquals($formattedName, $actualValue);
        $this->assertTrue($transport->isExhausted());
    }
    ...
    /** @test */
    public function readReportTest()
    {
        $transport = $this->createTransport();
        $gapicClient = $this->createClient([
            'transport' => $transport,
        ...
        // Mock response
        $nextPageToken = '';
        $rowsElement = new ListValue();
        $rows = [$rowsElement];
        $expectedResponse = new ReadReportResponse();
        $expectedResponse->setNextPageToken($nextPageToken);
        $expectedResponse->setRows($rows);
        $transport->addResponse($expectedResponse);
        // Mock request
        $formattedName = $gapicClient->reportName('[PROJECT]', '[LOCATION]', '[REPORT]');
        $request = (new ReadReportRequest())->setName($formattedName);
        $response = $gapicClient->readReport($request);
        $this->assertEquals($expectedResponse, $response->getPage()->getResponseObject());
        $resources = iterator_to_array($response->iterateAllElements());
        $this->assertSame(1, count($resources));
        ...
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.cloud.appoptimize.v1beta.AppOptimize/ReadReport', $actualFuncCall);
        $actualValue = $actualRequestObject->getName();
        $this->assertProtobufEquals($formattedName, $actualValue);
        $this->assertTrue($transport->isExhausted());
    }
    ...
    /** @test */
    public function readReportExceptionTest()
    {
        $transport = $this->createTransport();
        $gapicClient = $this->createClient([
            'transport' => $transport,
        ...
        $request = (new ReadReportRequest())->setName($formattedName);
        try {
            $gapicClient->readReport($request);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
