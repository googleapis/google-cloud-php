 ...
 */
function sync_workspace_refs_sample(string $formattedName): void
{
    // Create a client.
    $dataformClient = new DataformClient();

    // Prepare the request message.
    $request = (new SyncWorkspaceRefsRequest())
        ->setName($formattedName);

    // Call the API and handle any network failures.
    try {
        /** @var SyncWorkspaceRefsResponse $response */
        $response = $dataformClient->syncWorkspaceRefs($request);
...
function callSample(): void
{
    ...
    sync_workspace_refs_sample($formattedName);
}
