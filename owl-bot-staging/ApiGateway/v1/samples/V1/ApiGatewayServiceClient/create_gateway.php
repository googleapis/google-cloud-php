 ...
 *                                   the parent resource.
 * @param string $formattedApiConfig Resource name of the API Config for this Gateway.
...
function create_gateway_sample(
    ...
    $gateway = (new Gateway())
        ->setApiConfig($gatewayApiConfig);

    // Call the API and handle any network failures.
    try {
        /** @var OperationResponse $response */
        $response = $apiGatewayServiceClient->createGateway(
            $formattedParent,
            $gatewayId,
            $gateway,
            $formattedApiConfig
...
function callSample(): void
{
    ...
    $formattedApiConfig = ApiGatewayServiceClient::apiConfigName('[PROJECT]', '[API]', '[API_CONFIG]');

    create_gateway_sample($formattedParent, $gatewayId, $formattedApiConfig);
}
