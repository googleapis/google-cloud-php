 ...
 */
function get_content_bundle_sample(string $formattedName): void
{
    // Create a client.
    $contentBundleServiceClient = new ContentBundleServiceClient();

    // Prepare the request message.
    $request = (new GetContentBundleRequest())
        ->setName($formattedName);

    // Call the API and handle any network failures.
    try {
        /** @var ContentBundle $response */
        $response = $contentBundleServiceClient->getContentBundle($request);
...
{
    $formattedName = ContentBundleServiceClient::contentBundleName(
        '[NETWORK_CODE]',
        '[CONTENT_BUNDLE]'
    );

    get_content_bundle_sample($formattedName);
}
