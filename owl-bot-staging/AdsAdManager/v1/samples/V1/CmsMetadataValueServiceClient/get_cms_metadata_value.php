 ...
 */
function get_cms_metadata_value_sample(string $formattedName): void
{
    // Create a client.
    ...
    try {
        /** @var CmsMetadataValue $response */
        $response = $cmsMetadataValueServiceClient->getCmsMetadataValue($request);
...
{
    $formattedName = CmsMetadataValueServiceClient::cmsMetadataValueName(
        '[NETWORK_CODE]',
        '[CMS_METADATA_VALUE]'
    );

    get_cms_metadata_value_sample($formattedName);
}
