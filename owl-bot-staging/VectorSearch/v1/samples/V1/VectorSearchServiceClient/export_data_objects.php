 ...
 *
 * @param string $formattedName The resource name of the Collection from which we want to export
 *                              Data Objects. Format:
 ...
 */
function export_data_objects_sample(string $formattedName): void
{
    // Create a client.
    $vectorSearchServiceClient = new VectorSearchServiceClient();

    // Prepare the request message.
    $request = (new ExportDataObjectsRequest())
        ->setName($formattedName);

    // Call the API and handle any network failures.
    try {
        /** @var OperationResponse $response */
        $response = $vectorSearchServiceClient->exportDataObjects($request);
        $response->pollUntilComplete();

        if ($response->operationSucceeded()) {
            /** @var ExportDataObjectsResponse $result */
...
function callSample(): void
{
    ...
    export_data_objects_sample($formattedName);
}
