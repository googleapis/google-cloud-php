 ...
 */
function execute_tool_sample(string $formattedParent): void
{
    // Create a client.
    $toolServiceClient = new ToolServiceClient();

    // Prepare the request message.
    $request = (new ExecuteToolRequest())
        ->setParent($formattedParent);

    // Call the API and handle any network failures.
    try {
        /** @var ExecuteToolResponse $response */
        $response = $toolServiceClient->executeTool($request);
...
function callSample(): void
{
    ...
    execute_tool_sample($formattedParent);
}
