 ...
 *
 * @param string $formattedName     The name of the hub to reject spoke update. Please see
 *                                  {@see HubServiceClient::hubName()} for help formatting this field.
 * @param string $formattedSpokeUri The URI of the spoke to reject update. Please see
 *                                  {@see HubServiceClient::spokeName()} for help formatting this field.
 * @param string $spokeEtag         The etag of the spoke to reject update.
 */
function reject_spoke_update_sample(
    ...
    try {
        ...
        $response = $hubServiceClient->rejectSpokeUpdate($request);
        $response->pollUntilComplete();

        if ($response->operationSucceeded()) {
            /** @var RejectSpokeUpdateResponse $result */
...
function callSample(): void
{
    ...
    reject_spoke_update_sample($formattedName, $formattedSpokeUri, $spokeEtag);
}
