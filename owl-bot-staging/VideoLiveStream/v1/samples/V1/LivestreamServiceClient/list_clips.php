 ...
 *
 * @param string $formattedParent Parent value for ListClipsRequest
 *                                Please see {@see LivestreamServiceClient::channelName()} for help formatting this field.
 */
function list_clips_sample(string $formattedParent): void
{
    // Create a client.
    $livestreamServiceClient = new LivestreamServiceClient();

    // Prepare the request message.
    $request = (new ListClipsRequest())
        ->setParent($formattedParent);

    // Call the API and handle any network failures.
    try {
        /** @var PagedListResponse $response */
        $response = $livestreamServiceClient->listClips($request);
...
function callSample(): void
{
    ...
    list_clips_sample($formattedParent);
}
