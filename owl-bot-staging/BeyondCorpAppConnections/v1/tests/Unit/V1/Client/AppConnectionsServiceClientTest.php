...
namespace Google\Cloud\BeyondCorp\AppConnections\Tests\Unit\V1\Client;
...
class AppConnectionsServiceClientTest extends GeneratedTest
{
    ...
    public function resolveAppConnectionsTest()
    {
        ...
        $response = $gapicClient->resolveAppConnections($request);
        $this->assertEquals($expectedResponse, $response->getPage()->getResponseObject());
        $resources = iterator_to_array($response->iterateAllElements());
        $this->assertSame(1, count($resources));
    ...
    public function resolveAppConnectionsExceptionTest()
    {
        ...
        $request = (new ResolveAppConnectionsRequest())
            ...
            ->setAppConnectorId($formattedAppConnectorId);
        try {
            $gapicClient->resolveAppConnections($request);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
    ...
    /** @test */
    public function createAppConnectionAsyncTest()
    {
        $operationsTransport = $this->createTransport();
        $operationsClient = new OperationsClient([
            'apiEndpoint' => '',
