...
// [START apihub_v1_generated_Provisioning_CreateApiHubInstance_sync]
use Google\ApiCore\ApiException;
use Google\ApiCore\OperationResponse;
use Google\Cloud\ApiHub\V1\ApiHubInstance;
use Google\Cloud\ApiHub\V1\ApiHubInstance\Config;
...
function create_api_hub_instance_sample(
    ...
    try {
        ...
        $response = $provisioningClient->createApiHubInstance($request);
        $response->pollUntilComplete();

        if ($response->operationSucceeded()) {
            /** @var ApiHubInstance $result */
