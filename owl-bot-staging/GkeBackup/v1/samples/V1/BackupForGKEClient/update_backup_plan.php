 ...
 * Update a BackupPlan.
 *
 * @param string $formattedCluster Immutable. The source cluster from which Backups will be created via
 *                                 this BackupPlan.
 *                                 Valid formats:
 *
 ...
 */
function update_backup_plan_sample(string $formattedCluster): void
{
    // Create a client.
    ...
    $backupPlan = (new BackupPlan())
        ->setCluster($backupPlanCluster);

    // Call the API and handle any network failures.
    try {
        /** @var OperationResponse $response */
        $response = $backupForGKEClient->updateBackupPlan($backupPlan, $formattedCluster);
...
function callSample(): void
{
    ...
    update_backup_plan_sample($formattedCluster);
}
