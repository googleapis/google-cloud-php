 ...
 * completed.
 *
 * @param string $formattedParent               The project to clone the database in. Format is
 ...
 * @param string $formattedPitrSnapshotDatabase The name of the database that this was a snapshot of. Format:
 *                                              `projects/{project}/databases/{database}`. Please see
 *                                              {@see FirestoreAdminClient::databaseName()} for help formatting this field.
 */
function clone_database_sample(
    string $formattedParent,
    string $databaseId,
    string $formattedPitrSnapshotDatabase
): void {
    // Create a client.
    ...
    $pitrSnapshot = (new PitrSnapshot())
        ->setDatabase($formattedPitrSnapshotDatabase)
        ->setSnapshotTime($pitrSnapshotSnapshotTime);
    ...
    try {
        ...
        $response = $firestoreAdminClient->cloneDatabase($request);
...
function callSample(): void
{
    ...
    $formattedPitrSnapshotDatabase = FirestoreAdminClient::databaseName('[PROJECT]', '[DATABASE]');

    clone_database_sample($formattedParent, $databaseId, $formattedPitrSnapshotDatabase);
}
