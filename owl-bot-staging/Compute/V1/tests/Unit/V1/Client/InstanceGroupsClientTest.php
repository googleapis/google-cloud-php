...
class InstanceGroupsClientTest extends GeneratedTest
{
    ...
    public function addInstancesTest()
    {
        ...
        $request = (new AddInstancesInstanceGroupRequest())
            ...
            ->setProject($project)
            ->setZone($zone);
        $response = $gapicClient->addInstances($request);
    ...
    public function removeInstancesTest()
    {
        ...
        $request = (new RemoveInstancesInstanceGroupRequest())
            ...
            ->setProject($project)
            ->setZone($zone);
        $response = $gapicClient->removeInstances($request);
        $this->assertFalse($response->isDone());
        $apiRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($apiRequests));
        $operationsRequestsEmpty = $operationsTransport->popReceivedCalls();
    ...
    /** @test */
    public function addInstancesAsyncTest()
    {
        $operationsTransport = $this->createTransport();
        $operationsClient = new ZoneOperationsClient([
            'apiEndpoint' => '',
        ...
        // Mock response
        $incompleteOperation = new Operation();
        $incompleteOperation->setName('customOperations/addInstancesAsyncTest');
        $incompleteOperation->setStatus(Status::RUNNING);
        $transport->addResponse($incompleteOperation);
        $completeOperation = new Operation();
        $completeOperation->setName('customOperations/addInstancesAsyncTest');
        $completeOperation->setStatus(Status::DONE);
        $operationsTransport->addResponse($completeOperation);
        // Mock request
        $instanceGroup = 'instanceGroup81095253';
