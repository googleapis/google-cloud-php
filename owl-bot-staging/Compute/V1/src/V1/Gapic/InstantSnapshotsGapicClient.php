 ...
 *     $pagedResponse = $instantSnapshotsClient->aggregatedList($project);
 ...
 *     }
 * } finally {
 *     $instantSnapshotsClient->close();
 * }
 * ```
 *
 * @deprecated Please use the new service client {@see \Google\Cloud\Compute\V1\Client\InstantSnapshotsClient}.
 */
class InstantSnapshotsGapicClient
{
    use GapicClientTrait;

    /** The name of the service. */
    const SERVICE_NAME = 'google.cloud.compute.v1.InstantSnapshots';
    ...
    public function aggregatedList($project, array $optionalArgs = [])
    {
        ...
        return $this->getPagedListResponse('AggregatedList', $optionalArgs, InstantSnapshotAggregatedList::class, $request);
    }
     ...
     *     $project = 'project';
     *     $zone = 'zone';
     *     $operationResponse = $instantSnapshotsClient->delete($instantSnapshot, $project, $zone);
     ...
     *     // ... do other work
     *     $newOperationResponse = $instantSnapshotsClient->resumeOperation($operationName, 'delete');
     ...
     * @throws ApiException if the remote call fails
     */
    public function delete($instantSnapshot, $project, $zone, array $optionalArgs = [])
    {
     ...
     * @throws ApiException if the remote call fails
     */
    public function get($instantSnapshot, $project, $zone, array $optionalArgs = [])
    {
     ...
     *     $project = 'project';
     *     $zone = 'zone';
     *     $operationResponse = $instantSnapshotsClient->insert($instantSnapshotResource, $project, $zone);
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
     *         // if creating/modifying, retrieve the target resource
     *     } else {
     ...
     *     $operationName = $operationResponse->getName();
     *     // ... do other work
     *     $newOperationResponse = $instantSnapshotsClient->resumeOperation($operationName, 'insert');
     *     while (!$newOperationResponse->isDone()) {
     *         // ... do other work
     *         $newOperationResponse->reload();
     *     }
     ...
     * @throws ApiException if the remote call fails
     */
    public function insert($instantSnapshotResource, $project, $zone, array $optionalArgs = [])
    {
     ...
     *     $zone = 'zone';
     *     // Iterate over pages of elements
     *     $pagedResponse = $instantSnapshotsClient->list($project, $zone);
     *     foreach ($pagedResponse->iteratePages() as $page) {
     ...
     *     $response = $instantSnapshotsClient->setIamPolicy($project, $resource, $zone, $zoneSetPolicyRequestResource);
     * } finally {
     *     $instantSnapshotsClient->close();
     * }
     * ```
     ...
     *     $operationResponse = $instantSnapshotsClient->setLabels($project, $resource, $zone, $zoneSetLabelsRequestResource);
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
     *         // if creating/modifying, retrieve the target resource
     *     } else {
     ...
     *     $operationName = $operationResponse->getName();
     *     // ... do other work
     *     $newOperationResponse = $instantSnapshotsClient->resumeOperation($operationName, 'setLabels');
     *     while (!$newOperationResponse->isDone()) {
     *         // ... do other work
     *         $newOperationResponse->reload();
     *     }
     ...
     *     $zone = 'zone';
     *     $response = $instantSnapshotsClient->testIamPermissions($project, $resource, $testPermissionsRequestResource, $zone);
     * } finally {
     *     $instantSnapshotsClient->close();
     * }
     * ```
