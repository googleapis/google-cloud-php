 ...
 *     $project = 'project';
 *     $region = 'region';
 *     $operationResponse = $regionInstantSnapshotsClient->delete($instantSnapshot, $project, $region);
 ...
 *     // ... do other work
 *     $newOperationResponse = $regionInstantSnapshotsClient->resumeOperation($operationName, 'delete');
 ...
 *     }
 * } finally {
 *     $regionInstantSnapshotsClient->close();
 * }
 * ```
 *
 * @deprecated Please use the new service client {@see \Google\Cloud\Compute\V1\Client\RegionInstantSnapshotsClient}.
 */
class RegionInstantSnapshotsGapicClient
{
    use GapicClientTrait;

    /** The name of the service. */
    const SERVICE_NAME = 'google.cloud.compute.v1.RegionInstantSnapshots';
     ...
     */
    public function delete($instantSnapshot, $project, $region, array $optionalArgs = [])
    {
        $request = new DeleteRegionInstantSnapshotRequest();
        $requestParamHeaders = [];
        $request->setInstantSnapshot($instantSnapshot);
        $request->setProject($project);
        $request->setRegion($region);
        $requestParamHeaders['instant_snapshot'] = $instantSnapshot;
     ...
     * @throws ApiException if the remote call fails
     */
    public function get($instantSnapshot, $project, $region, array $optionalArgs = [])
    {
        ...
        $requestParams = new RequestParamsHeaderDescriptor($requestParamHeaders);
        $optionalArgs['headers'] = isset($optionalArgs['headers']) ? array_merge($requestParams->getHeader(), $optionalArgs['headers']) : $requestParams->getHeader();
        return $this->startCall('Get', InstantSnapshot::class, $optionalArgs, $request)->wait();
    }
     ...
     *     $project = 'project';
     *     $region = 'region';
     *     $operationResponse = $regionInstantSnapshotsClient->insert($instantSnapshotResource, $project, $region);
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
     *         // if creating/modifying, retrieve the target resource
     *     } else {
     ...
     *     $operationName = $operationResponse->getName();
     *     // ... do other work
     *     $newOperationResponse = $regionInstantSnapshotsClient->resumeOperation($operationName, 'insert');
     *     while (!$newOperationResponse->isDone()) {
     *         // ... do other work
     *         $newOperationResponse->reload();
     *     }
     ...
     * ```
     *
     * @param InstantSnapshot $instantSnapshotResource The body resource for this request
     * @param string          $project                 Project ID for this request.
     ...
     * @throws ApiException if the remote call fails
     */
    public function insert($instantSnapshotResource, $project, $region, array $optionalArgs = [])
    {
     ...
     *     $pagedResponse = $regionInstantSnapshotsClient->list($project, $region);
    ...
    public function list($project, $region, array $optionalArgs = [])
    {
        ...
        $requestParams = new RequestParamsHeaderDescriptor($requestParamHeaders);
        $optionalArgs['headers'] = isset($optionalArgs['headers']) ? array_merge($requestParams->getHeader(), $optionalArgs['headers']) : $requestParams->getHeader();
        return $this->getPagedListResponse('List', $optionalArgs, InstantSnapshotList::class, $request);
    }
     ...
     *     $resource = 'resource';
     *     $response = $regionInstantSnapshotsClient->setIamPolicy($project, $region, $regionSetPolicyRequestResource, $resource);
     * } finally {
     *     $regionInstantSnapshotsClient->close();
     * }
     * ```
     ...
     *     $resource = 'resource';
     *     $operationResponse = $regionInstantSnapshotsClient->setLabels($project, $region, $regionSetLabelsRequestResource, $resource);
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
     *         // if creating/modifying, retrieve the target resource
     *     } else {
     ...
     *     $operationName = $operationResponse->getName();
     *     // ... do other work
     *     $newOperationResponse = $regionInstantSnapshotsClient->resumeOperation($operationName, 'setLabels');
     *     while (!$newOperationResponse->isDone()) {
     *         // ... do other work
     *         $newOperationResponse->reload();
     *     }
     ...
     *     $response = $regionInstantSnapshotsClient->testIamPermissions($project, $region, $resource, $testPermissionsRequestResource);
     * } finally {
     *     $regionInstantSnapshotsClient->close();
     * }
     * ```
