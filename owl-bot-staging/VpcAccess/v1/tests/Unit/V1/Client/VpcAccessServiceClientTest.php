...
namespace Google\Cloud\VpcAccess\Tests\Unit\V1\Client;
 ...
 * @group gapic
 */
class VpcAccessServiceClientTest extends GeneratedTest
{
    ...
    /** @test */
    public function createConnectorAsyncTest()
    {
        $operationsTransport = $this->createTransport();
        $operationsClient = new OperationsClient([
            'apiEndpoint' => '',
        ...
        $request = (new CreateConnectorRequest())
            ...
            ->setConnectorId($connectorId)
            ->setConnector($connector);
        $response = $gapicClient->createConnectorAsync($request)->wait();
        $this->assertFalse($response->isDone());
        $this->assertNull($response->getResult());
        $apiRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($apiRequests));
