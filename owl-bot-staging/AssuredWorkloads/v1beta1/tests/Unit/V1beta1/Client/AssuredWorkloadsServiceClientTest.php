...
namespace Google\Cloud\AssuredWorkloads\Tests\Unit\V1beta1\Client;
...
class AssuredWorkloadsServiceClientTest extends GeneratedTest
{
    ...
    public function analyzeWorkloadMoveTest()
    {
        ...
        $request = (new AnalyzeWorkloadMoveRequest())
            ->setTarget($target);
        $response = $gapicClient->analyzeWorkloadMove($request);
    ...
    public function analyzeWorkloadMoveExceptionTest()
    {
        ...
        $request = (new AnalyzeWorkloadMoveRequest())
            ->setTarget($target);
        try {
            $gapicClient->analyzeWorkloadMove($request);
    ...
    /** @test */
    public function analyzeWorkloadMoveAsyncTest()
    {
        $transport = $this->createTransport();
        $gapicClient = $this->createClient([
            'transport' => $transport,
        ...
        $request = (new AnalyzeWorkloadMoveRequest())
            ->setTarget($target);
        $response = $gapicClient->analyzeWorkloadMoveAsync($request)->wait();
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
