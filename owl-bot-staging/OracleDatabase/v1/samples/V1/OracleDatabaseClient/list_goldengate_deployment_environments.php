 ...
 */
function list_goldengate_deployment_environments_sample(string $formattedParent): void
{
    // Create a client.
    $oracleDatabaseClient = new OracleDatabaseClient();

    // Prepare the request message.
    $request = (new ListGoldengateDeploymentEnvironmentsRequest())
        ->setParent($formattedParent);

    // Call the API and handle any network failures.
    try {
        /** @var PagedListResponse $response */
        $response = $oracleDatabaseClient->listGoldengateDeploymentEnvironments($request);

        /** @var GoldengateDeploymentEnvironment $element */
...
function callSample(): void
{
    ...
    list_goldengate_deployment_environments_sample($formattedParent);
}
