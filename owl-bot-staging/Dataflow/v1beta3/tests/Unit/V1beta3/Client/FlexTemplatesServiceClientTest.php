...
class FlexTemplatesServiceClientTest extends GeneratedTest
{
    ...
    public function launchFlexTemplateTest()
    {
        ...
        $response = $gapicClient->launchFlexTemplate($request);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.dataflow.v1beta3.FlexTemplatesService/LaunchFlexTemplate', $actualFuncCall);
        $this->assertTrue($transport->isExhausted());
    }
    ...
    public function launchFlexTemplateExceptionTest()
    {
        ...
        $request = new LaunchFlexTemplateRequest();
        try {
            $gapicClient->launchFlexTemplate($request);
    ...
    /** @test */
    public function launchFlexTemplateAsyncTest()
    {
        $transport = $this->createTransport();
        $gapicClient = $this->createClient([
            'transport' => $transport,
        ...
        $transport->addResponse($expectedResponse);
        $request = new LaunchFlexTemplateRequest();
        $response = $gapicClient->launchFlexTemplateAsync($request)->wait();
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
