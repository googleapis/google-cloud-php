...
use Google\Cloud\Spanner\Admin\Database\V1\SplitPoints\Key;
use Google\Protobuf\ListValue;

/**
 * Adds split points to specified tables, indexes of a database.
 *
 * @param string $formattedDatabase The database on whose tables/indexes split points are to be
 ...
 */
function add_split_points_sample(string $formattedDatabase): void
{
    // Create a client.
    ...
    try {
        /** @var AddSplitPointsResponse $response */
        $response = $databaseAdminClient->addSplitPoints($request);
...
function callSample(): void
{
    ...
    add_split_points_sample($formattedDatabase);
}
