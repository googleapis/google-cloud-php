 ...
 * @param string $formattedModel    The resource name of the Model that this is the deployment of. Note that
 ...
 */
function deploy_model_sample(string $formattedEndpoint, string $formattedModel): void
{
    // Create a client.
    ...
    $deployedModel = (new DeployedModel())
        ->setModel($deployedModelModel);

    // Call the API and handle any network failures.
    try {
        /** @var OperationResponse $response */
        $response = $endpointServiceClient->deployModel(
            $formattedEndpoint,
            $deployedModel,
            $formattedModel
        );
        ...
        if ($response->operationSucceeded()) {
            /** @var DeployModelResponse $response */
...
function callSample(): void
{
    ...
    $formattedModel = EndpointServiceClient::modelName('[PROJECT]', '[LOCATION]', '[MODEL]');

    deploy_model_sample($formattedEndpoint, $formattedModel);
}
