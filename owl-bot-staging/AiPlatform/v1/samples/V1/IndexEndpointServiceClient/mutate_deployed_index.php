 ...
 * @param string $formattedIndex         The name of the Index this is the deployment of.
...
function mutate_deployed_index_sample(
    ...
    try {
        ...
        if ($response->operationSucceeded()) {
            /** @var MutateDeployedIndexResponse $response */
...
function callSample(): void
{
    ...
    $deployedIndexId = '[ID]';
    $formattedIndex = IndexEndpointServiceClient::indexName('[PROJECT]', '[LOCATION]', '[INDEX]');

    mutate_deployed_index_sample($formattedIndexEndpoint, $deployedIndexId, $formattedIndex);
}
