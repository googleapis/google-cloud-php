...
class Bucket
{
     ...
     * @param array $data
     */
    public function __construct(ConnectionInterface $connection, $name, $data = null)
    {
     ...
     * @return Object
     */
    public function uploadFromFile($body, $destination, $options = [])
    {
     ...
     * @return Object
     */
    public function uploadFromPath($path, $options = [])
    {
    ...
    {
        $objectIdentity = $this->identity + [
            'object' => $objectName,
            'generation' => isset($options['generation']) ? $options['generation'] : null
        ];

        return new Object($this->connection, $objectIdentity);
    }
     ...
     *     @type string $fields Selector which will triggers response to return
    ...
    public function objects(array $options = [])
    {
        ...
        do {
            $response = $this->connection->listObjects($options + ['bucket' => $this->name]);

            foreach ($response['items'] as $object) {
                $objectIdentity = $this->identity + ['object' => $object['name']];

                yield new Object($this->connection, $objectIdentity, $object);
            }
