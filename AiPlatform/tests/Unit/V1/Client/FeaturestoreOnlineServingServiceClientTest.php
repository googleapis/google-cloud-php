...
class FeaturestoreOnlineServingServiceClientTest extends GeneratedTest
{
    ...
    public function readFeatureValuesTest()
    {
        ...
        $response = $gapicClient->readFeatureValues($request);
    ...
    public function readFeatureValuesExceptionTest()
    {
        ...
        $request = (new ReadFeatureValuesRequest())
            ...
            ->setFeatureSelector($featureSelector);
        try {
            $gapicClient->readFeatureValues($request);
    ...
    public function streamingReadFeatureValuesTest()
    {
        ...
        $request = (new StreamingReadFeatureValuesRequest())
            ...
            ->setFeatureSelector($featureSelector);
        $serverStream = $gapicClient->streamingReadFeatureValues($request);
    ...
    public function writeFeatureValuesTest()
    {
        ...
        $payloads = [];
        $request = (new WriteFeatureValuesRequest())->setEntityType($formattedEntityType)->setPayloads($payloads);
        $response = $gapicClient->writeFeatureValues($request);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
    ...
    public function writeFeatureValuesExceptionTest()
    {
        ...
        $request = (new WriteFeatureValuesRequest())->setEntityType($formattedEntityType)->setPayloads($payloads);
        try {
            $gapicClient->writeFeatureValues($request);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
    ...
    /** @test */
    public function readFeatureValuesAsyncTest()
    {
        $transport = $this->createTransport();
        $gapicClient = $this->createClient([
            'transport' => $transport,
        ...
        $request = (new ReadFeatureValuesRequest())
            ...
            ->setEntityId($entityId)
            ->setFeatureSelector($featureSelector);
        $response = $gapicClient->readFeatureValuesAsync($request)->wait();
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
