 ...
 */
function query_reasoning_engine_sample(string $formattedName): void
{
    // Create a client.
    $reasoningEngineExecutionServiceClient = new ReasoningEngineExecutionServiceClient();

    // Prepare the request message.
    $request = (new QueryReasoningEngineRequest())
        ->setName($formattedName);

    // Call the API and handle any network failures.
    try {
        /** @var QueryReasoningEngineResponse $response */
        $response = $reasoningEngineExecutionServiceClient->queryReasoningEngine($request);
...
function callSample(): void
{
    ...
    query_reasoning_engine_sample($formattedName);
}
