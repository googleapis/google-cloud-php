 ...
 */
function stream_query_reasoning_engine_sample(string $formattedName): void
{
    // Create a client.
    $reasoningEngineExecutionServiceClient = new ReasoningEngineExecutionServiceClient();

    // Prepare the request message.
    $request = (new StreamQueryReasoningEngineRequest())
        ->setName($formattedName);

    // Call the API and handle any network failures.
    try {
        /** @var ServerStream $stream */
        $stream = $reasoningEngineExecutionServiceClient->streamQueryReasoningEngine($request);
...
function callSample(): void
{
    ...
    stream_query_reasoning_engine_sample($formattedName);
}
