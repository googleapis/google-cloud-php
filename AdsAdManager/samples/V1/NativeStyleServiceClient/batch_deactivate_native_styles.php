 ...
 * @param string $formattedNamesElement Resource names for the `NativeStyle`s.
 ...
 */
function batch_deactivate_native_styles_sample(
    ...
    $request = (new BatchDeactivateNativeStylesRequest())
        ->setParent($formattedParent)
        ->setNames($formattedNames);

    // Call the API and handle any network failures.
    try {
        /** @var BatchDeactivateNativeStylesResponse $response */
        $response = $nativeStyleServiceClient->batchDeactivateNativeStyles($request);
...
function callSample(): void
{
    ...
    $formattedNamesElement = NativeStyleServiceClient::nativeStyleName(
        '[NETWORK_CODE]',
        '[NATIVE_STYLE]'
    );

    batch_deactivate_native_styles_sample($formattedParent, $formattedNamesElement);
}
