 ...
 *
 * @param string $formattedParent                              The parent resource where `NativeStyles` will be created.
 ...
 * @param string $formattedRequestsParent                      The parent resource where this `NativeStyle` will be created.
 *                                                             Format: `networks/{network_code}`
 ...
 *                                                             [AdUnit][google.ads.admanager.v1.AdUnit], or LineItem.
 */
function batch_create_native_styles_sample(
    ...
    $createNativeStyleRequest = (new CreateNativeStyleRequest())
        ->setParent($formattedRequestsParent)
        ->setNativeStyle($requestsNativeStyle);
    $requests = [$createNativeStyleRequest,];
    $request = (new BatchCreateNativeStylesRequest())
        ->setParent($formattedParent)
        ->setRequests($requests);

    // Call the API and handle any network failures.
    try {
        /** @var BatchCreateNativeStylesResponse $response */
        $response = $nativeStyleServiceClient->batchCreateNativeStyles($request);
...
function callSample(): void
{
    ...
    $formattedRequestsParent = NativeStyleServiceClient::networkName('[NETWORK_CODE]');
    ...
    batch_create_native_styles_sample(
        $formattedParent,
