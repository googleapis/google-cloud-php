 ...
 */
function batch_create_companies_sample(
    ...
    try {
        /** @var BatchCreateCompaniesResponse $response */
        $response = $companyServiceClient->batchCreateCompanies($request);
...
function callSample(): void
{
    ...
    $formattedRequestsParent = CompanyServiceClient::networkName('[NETWORK_CODE]');
    $requestsCompanyDisplayName = '[DISPLAY_NAME]';
    $requestsCompanyType = CompanyType::COMPANY_TYPE_UNSPECIFIED;

    batch_create_companies_sample(
        $formattedParent,
        $formattedRequestsParent,
        $requestsCompanyDisplayName,
        $requestsCompanyType
    );
}
