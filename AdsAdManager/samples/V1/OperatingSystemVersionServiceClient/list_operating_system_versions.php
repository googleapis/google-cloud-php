 ...
 *                                Please see {@see OperatingSystemVersionServiceClient::networkName()} for help formatting this field.
 */
function list_operating_system_versions_sample(string $formattedParent): void
{
    // Create a client.
    $operatingSystemVersionServiceClient = new OperatingSystemVersionServiceClient();

    // Prepare the request message.
    $request = (new ListOperatingSystemVersionsRequest())
        ->setParent($formattedParent);

    // Call the API and handle any network failures.
    try {
        /** @var PagedListResponse $response */
        $response = $operatingSystemVersionServiceClient->listOperatingSystemVersions($request);
...
{
    $formattedParent = OperatingSystemVersionServiceClient::networkName('[NETWORK_CODE]');

    list_operating_system_versions_sample($formattedParent);
}
